...
Database.prototype.init = function *() {
	...
	for (var k in this.$config.database.persistences) {
		...
		var schema = new Schema(persistence.driver, persistence.connection);

	  	var models =  yield this.getModels(persistence.dir, schema);


		cachePersistences[name] = Persistence(schema, models, persistence.default);
		...
		for (var i = 0, l = models.length; i < l; i++) {
			var model = models[i];
			var name = S(model.shortPath.replace('/', '_')).camelize().s;
			this.$injector.register('$' + name + 'Repository', model.schema);
		}
...
Database.prototype.getModels = function *(modelsDir, schema) {
	...
	files.forEach(function (file) {
		if (~file.indexOf('.js')) {
			models.push({
				...
				schema: this.$injector.processInject(
					require(path.resolve(appDir, 'app/models', modelsDir, file))
				)(schema)
			});
		}
	}, this);
