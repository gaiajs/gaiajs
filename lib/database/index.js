...
Database.prototype.init = function *() {
	...
	for (var k in this.$config.database.persistences) {
	  ...
	  var models =  yield this.getModels(persistence.dir);
		...
		cachePersistences[name] = Persistence(persistenceDefinition, models);

		this.$injector.register('$' + name + 'Persistence', driver.native.api);
		if (persistence.default) this.$injector.register('$persistence', driver.native.api);
		...
		if (persistenceDefinition.repositories && 1 === this.$config.database.persistences.length) {
			for (var repoName in persistenceDefinition.repositories) {
				console.log('Register ', repoName);
				this.$injector.register('$' + repoName + 'Repository', driver.native.api);
			}
		}
