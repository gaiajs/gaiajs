...
const filesConfig = [{
    ...
    name: 'database',
    require: false,
    'normalizer': './normalizers/databaseNormalizer'
}, {
    name: 'route',
    require: true,
    'normalizer': './normalizers/routeNormalizer'
}, {
    name: 'authentification',
    require: false,
    'normalizer': './normalizers/authentificationNormalizer'
}, {
    name: 'filters',
    require: false
}, {
    ...
    name: 'socket',
    require: false,
    'normalizer': './normalizers/socketNormalizer'
}, {
    name: 'generator',
    require: false,
    'normalizer': './normalizers/generatorNormalizer'
}];
 ...
 ** Class
 ********************************/
function Config($log, $configPath, $injector) {
    this.$log = $log;
...
Config.$inject = ["$log", "$configPath", "$injector"];
...
Config.prototype._loadConfig = function * (env) {
    ...
    for (var k in files) {
        ...
        try {
            ...
            if (fileConfig.normalizer) {
                var normalizer = this.$injector.processInject(require(fileConfig.normalizer));
                conf = normalizer(file, conf);
            }
            this[fileName] = conf;
        } catch (err) {
