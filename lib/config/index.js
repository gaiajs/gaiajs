...
const filesConfig = [
  {name: 'application', require: true, 'normalizer': require('./normalizers/applicationNormalizer')},
  ...
  {name: 'socket', require: false, 'normalizer': require('./normalizers/socketNormalizer')},
...
Config.prototype.initialize = function *(appDir) {
  ...
  this.appDir = appDir;
  this.configDir = process.env.GAIAJS_CONFIG_DIR || path.resolve(appDir, 'config/');
...
Config.prototype._loadConfig = function *(env) {
  ...
  if (!fs.existsSync(configPath)) {
    return console.warn("The folder " + configPath + " does not exist");
  }
