...
Gaia.prototype.initMiddleware = function *() {
	...
	if (config.application.i18n) {
		...
		this._app.use(i18n(this._app, config.application.i18n));
	}
...
Gaia.prototype.start = function() {
	var self = this;
	co(function *() {
		yield config.initialize(self._appDir);
		self.config = config;
		self._app = koa();
		...
		if (self._appConfig.beforeMiddlewares && _.isFunction(self._appConfig.beforeMiddlewares)) {
			self.log.info("Call beforeMiddlewares");
			yield self._appConfig.beforeMiddlewares.call(null, self._app);
		}
